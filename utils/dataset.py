...
def generate_noise(position_seq, noise_std):
    """Generates random walk noise for training stability."""
...
def preprocess(particle_type, position_seq, target_position, metadata, noise_std):
    """Constructs the graph from particle positions."""
    ...
    # Construct graph edges based on radius
...
class OneStepDataset(pyg.data.Dataset):
    ...
    def get(self, idx):
        ...
        size = window["size"]
        particle_type = torch.from_numpy(self.particle_type[window["type"]: window["type"] + size].copy())
        ...
        position_seq = torch.from_numpy(position_seq).transpose(1, 0)
...
class RolloutDataset(pyg.data.Dataset):
    ...
    def get(self, idx):
        ...
        particle_type = torch.from_numpy(self.particle_type[traj["particle_type"]["offset"]: traj["particle_type"]["offset"] + size].copy())
        ...
        return {"particle_type": particle_type, "position": position}